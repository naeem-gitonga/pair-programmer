...
- When given a URL in your instructions, read the content of that URL first. If the URL contains a code snippet, read it carefully and understand it and its implications and explain them.
...
- Before accepting what you have proposed to implement, read the relevant files to confirm that your proposed implementation is consistent with the existing codebase and won't cause conflicts or issues. If you find any inconsistencies, adjust your plan accordingly before proceeding. 
- Before accepting what you have proposed to implement, re-read your own proposed implementation to check for any logical errors, edge cases, or potential issues. If you find any, adjust your implementation accordingly before proceeding.
- Before writing any boolean flag, ref used as a guard, or conditional check, trace through its value for every scenario: (1) initial state / first use, (2) normal case, (3) the exact case you are guarding. If the value starts false and only becomes true under a specific condition, your code must work correctly when it stays false indefinitely.
- When fixing a bug or applying feedback: before touching any code, state in one sentence WHY the existing code is wrong — not what is wrong, but why. If you cannot state the why, re-read the code until you can. Apply the fix only after you understand the root cause. A fix that addresses the symptom without the cause will introduce a different bug.
- After writing a fix, re-read the changed code and answer two questions explicitly: (1) Does the original problem no longer exist? (2) Does anything you changed have a different meaning than what it replaced? If you renamed a variable, changed a condition, or moved logic, verify the new version is semantically equivalent in all cases — not just the one you were thinking about.
- After writing a new hook or utility function, search the codebase for a similar function or identical logic before finishing. If it already exists elsewhere, use the existing one — do not create a second copy.
